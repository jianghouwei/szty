/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataAihaoiptmp18.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.aihaoiptmp18_Dao;
import com.szty.aihao.core.aihaoiptmp18_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@AIHAOIPTMP18逻辑层接口
    *@作者：宋春林 
    */

	public class aihaoiptmp18_service
	{
        public  aihaoiptmp18_core _dal=classFactory.getaihaoiptmp18();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP18实体
         * @return 新插入记录的编号
         */
		public int insert_aihaoiptmp18 (aihaoiptmp18_Dao _AIHAOIPTMP18Model )  throws Exception{
         return _dal.insert_aihaoiptmp18( _AIHAOIPTMP18Model);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP18prrameter
         * @return 新插入记录的编号
         */
         public int insert_aihaoiptmp18(Object[] _para)  throws Exception{
        return _dal.insert_aihaoiptmp18( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP18实体
         * @return 影响的行数
         */
		public int update_aihaoiptmp18(aihaoiptmp18_Dao _AIHAOIPTMP18Model)  throws Exception{
        return _dal.update_aihaoiptmp18( _AIHAOIPTMP18Model);
		}
	
		

         /**
         *  删除数据表AIHAOIPTMP18中的一条记录
         * @param AIHAOIPTMP18实体
         * @return 新插入记录的编号
         */
		public int delete_aihaoiptmp18(int Id)  throws Exception{
        return _dal.delete_aihaoiptmp18( Id);
		}

		

       
          /**
         * 得到 aihaoiptmp18 数据实体
         * @param Id">Id
         * @return<aihaoiptmp18 数据实体
        * @throws Exception 
         */
		public aihaoiptmp18_Dao get_aihaoiptmp18Dao(int Id) throws Exception{
        return _dal.get_aihaoiptmp18Dao( Id);
		}
		
		    /**
         * 根据AIHAOIPTMP18返回的查询DataRow创建一个AIHAOIPTMP18Entity对象
         * @param AIHAOIPTMP18 row
         * @returnAIHAOIPTMP18List对象
        * @throws Exception 
         */
		public List<aihaoiptmp18_Dao> get_aihaoiptmp18_All() throws Exception{
            return _dal.get_aihaoiptmp18_All();
            }
		    /**
         * 根据AIHAOIPTMP18返回的查询DataRow创建一个AIHAOIPTMP18Entity对象
         * @param AIHAOIPTMP18 row
         * @returnAIHAOIPTMP18List对象
        * @throws Exception 
         */
		public List<aihaoiptmp18_Dao> get_aihaoiptmp18_All(String strWhere) throws Exception{
            return _dal.get_aihaoiptmp18_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<aihaoiptmp18_Dao> get_aihaoiptmp18_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_aihaoiptmp18_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据AIHAOIPTMP18返回的查询DataRow创建一个AIHAOIPTMP18Entity对象
         * @param AIHAOIPTMP18 row
         * @returnAIHAOIPTMP18Dictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, aihaoiptmp18_Dao> get_aihaoiptmp18_Dictionary(String strWhere) throws Exception{
            return _dal.get_aihaoiptmp18_Dictionary(strWhere);
            }
		 /**
         * 更新AIHAOIPTMP18字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_aihaoiptmp18_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_aihaoiptmp18_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新AIHAOIPTMP18Int型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_aihaoiptmp18_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_aihaoiptmp18_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新AIHAOIPTMP18IString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createaihaoiptmp18_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_aihaoiptmp18_UpdateString( FieldName, Value, sid);
            }
        
        
    }
