/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataAihaobusiness19.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.aihaobusiness19_Dao;
import com.szty.aihao.core.aihaobusiness19_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@AIHAOBUSINESS19逻辑层接口
    *@作者：宋春林 
    */

	public class aihaobusiness19_service
	{
        public  aihaobusiness19_core _dal=classFactory.getaihaobusiness19();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param AIHAOBUSINESS19实体
         * @return 新插入记录的编号
         */
		public int insert_aihaobusiness19 (aihaobusiness19_Dao _AIHAOBUSINESS19Model )  throws Exception{
         return _dal.insert_aihaobusiness19( _AIHAOBUSINESS19Model);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param AIHAOBUSINESS19prrameter
         * @return 新插入记录的编号
         */
         public int insert_aihaobusiness19(Object[] _para)  throws Exception{
        return _dal.insert_aihaobusiness19( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param AIHAOBUSINESS19实体
         * @return 影响的行数
         */
		public int update_aihaobusiness19(aihaobusiness19_Dao _AIHAOBUSINESS19Model)  throws Exception{
        return _dal.update_aihaobusiness19( _AIHAOBUSINESS19Model);
		}
	
		

         /**
         *  删除数据表AIHAOBUSINESS19中的一条记录
         * @param AIHAOBUSINESS19实体
         * @return 新插入记录的编号
         */
		public int delete_aihaobusiness19(int Sname)  throws Exception{
        return _dal.delete_aihaobusiness19( Sname);
		}

		

       
          /**
         * 得到 aihaobusiness19 数据实体
         * @param Sname">Sname
         * @return<aihaobusiness19 数据实体
        * @throws Exception 
         */
		public aihaobusiness19_Dao get_aihaobusiness19Dao(int Sname) throws Exception{
        return _dal.get_aihaobusiness19Dao( Sname);
		}
		
		    /**
         * 根据AIHAOBUSINESS19返回的查询DataRow创建一个AIHAOBUSINESS19Entity对象
         * @param AIHAOBUSINESS19 row
         * @returnAIHAOBUSINESS19List对象
        * @throws Exception 
         */
		public List<aihaobusiness19_Dao> get_aihaobusiness19_All() throws Exception{
            return _dal.get_aihaobusiness19_All();
            }
		    /**
         * 根据AIHAOBUSINESS19返回的查询DataRow创建一个AIHAOBUSINESS19Entity对象
         * @param AIHAOBUSINESS19 row
         * @returnAIHAOBUSINESS19List对象
        * @throws Exception 
         */
		public List<aihaobusiness19_Dao> get_aihaobusiness19_All(String strWhere) throws Exception{
            return _dal.get_aihaobusiness19_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<aihaobusiness19_Dao> get_aihaobusiness19_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_aihaobusiness19_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据AIHAOBUSINESS19返回的查询DataRow创建一个AIHAOBUSINESS19Entity对象
         * @param AIHAOBUSINESS19 row
         * @returnAIHAOBUSINESS19Dictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, aihaobusiness19_Dao> get_aihaobusiness19_Dictionary(String strWhere) throws Exception{
            return _dal.get_aihaobusiness19_Dictionary(strWhere);
            }
		 /**
         * 更新AIHAOBUSINESS19字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_aihaobusiness19_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_aihaobusiness19_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新AIHAOBUSINESS19Int型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_aihaobusiness19_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_aihaobusiness19_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新AIHAOBUSINESS19IString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createaihaobusiness19_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_aihaobusiness19_UpdateString( FieldName, Value, sid);
            }
        
        
    }
