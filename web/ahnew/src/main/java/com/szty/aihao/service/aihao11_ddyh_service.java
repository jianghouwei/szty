/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataAihao11_ddyh.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.aihao11_ddyh_Dao;
import com.szty.aihao.core.aihao11_ddyh_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@AIHAO11_DDYH逻辑层接口
    *@作者：宋春林 
    */

	public class aihao11_ddyh_service
	{
        public  aihao11_ddyh_core _dal=classFactory.getaihao11_ddyh();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param AIHAO11_DDYH实体
         * @return 新插入记录的编号
         */
		public int insert_aihao11_ddyh (aihao11_ddyh_Dao _AIHAO11_DDYHModel )  throws Exception{
         return _dal.insert_aihao11_ddyh( _AIHAO11_DDYHModel);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param AIHAO11_DDYHprrameter
         * @return 新插入记录的编号
         */
         public int insert_aihao11_ddyh(Object[] _para)  throws Exception{
        return _dal.insert_aihao11_ddyh( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param AIHAO11_DDYH实体
         * @return 影响的行数
         */
		public int update_aihao11_ddyh(aihao11_ddyh_Dao _AIHAO11_DDYHModel)  throws Exception{
        return _dal.update_aihao11_ddyh( _AIHAO11_DDYHModel);
		}
	
		

         /**
         *  删除数据表AIHAO11_DDYH中的一条记录
         * @param AIHAO11_DDYH实体
         * @return 新插入记录的编号
         */
		public int delete_aihao11_ddyh(int Id)  throws Exception{
        return _dal.delete_aihao11_ddyh( Id);
		}

		

       
          /**
         * 得到 aihao11_ddyh 数据实体
         * @param Id">Id
         * @return<aihao11_ddyh 数据实体
        * @throws Exception 
         */
		public aihao11_ddyh_Dao get_aihao11_ddyhDao(int Id) throws Exception{
        return _dal.get_aihao11_ddyhDao( Id);
		}
		
		    /**
         * 根据AIHAO11_DDYH返回的查询DataRow创建一个AIHAO11_DDYHEntity对象
         * @param AIHAO11_DDYH row
         * @returnAIHAO11_DDYHList对象
        * @throws Exception 
         */
		public List<aihao11_ddyh_Dao> get_aihao11_ddyh_All() throws Exception{
            return _dal.get_aihao11_ddyh_All();
            }
		    /**
         * 根据AIHAO11_DDYH返回的查询DataRow创建一个AIHAO11_DDYHEntity对象
         * @param AIHAO11_DDYH row
         * @returnAIHAO11_DDYHList对象
        * @throws Exception 
         */
		public List<aihao11_ddyh_Dao> get_aihao11_ddyh_All(String strWhere) throws Exception{
            return _dal.get_aihao11_ddyh_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<aihao11_ddyh_Dao> get_aihao11_ddyh_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_aihao11_ddyh_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据AIHAO11_DDYH返回的查询DataRow创建一个AIHAO11_DDYHEntity对象
         * @param AIHAO11_DDYH row
         * @returnAIHAO11_DDYHDictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, aihao11_ddyh_Dao> get_aihao11_ddyh_Dictionary(String strWhere) throws Exception{
            return _dal.get_aihao11_ddyh_Dictionary(strWhere);
            }
		 /**
         * 更新AIHAO11_DDYH字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_aihao11_ddyh_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_aihao11_ddyh_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新AIHAO11_DDYHInt型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_aihao11_ddyh_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_aihao11_ddyh_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新AIHAO11_DDYHIString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createaihao11_ddyh_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_aihao11_ddyh_UpdateString( FieldName, Value, sid);
            }
        
        
    }
