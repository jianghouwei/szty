/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataA.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.a_Dao;
import com.szty.aihao.core.a_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@A逻辑层接口
    *@作者：宋春林 
    */

	public class a_service
	{
        public  a_core _dal=classFactory.geta();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param A实体
         * @return 新插入记录的编号
         */
		public int insert_a (a_Dao _AModel )  throws Exception{
         return _dal.insert_a( _AModel);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param Aprrameter
         * @return 新插入记录的编号
         */
         public int insert_a(Object[] _para)  throws Exception{
        return _dal.insert_a( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param A实体
         * @return 影响的行数
         */
		public int update_a(a_Dao _AModel)  throws Exception{
        return _dal.update_a( _AModel);
		}
	
		

         /**
         *  删除数据表A中的一条记录
         * @param A实体
         * @return 新插入记录的编号
         */
		public int delete_a(int Busname)  throws Exception{
        return _dal.delete_a( Busname);
		}

		

       
          /**
         * 得到 a 数据实体
         * @param Busname">Busname
         * @return<a 数据实体
        * @throws Exception 
         */
		public a_Dao get_aDao(int Busname) throws Exception{
        return _dal.get_aDao( Busname);
		}
		
		    /**
         * 根据A返回的查询DataRow创建一个AEntity对象
         * @param A row
         * @returnAList对象
        * @throws Exception 
         */
		public List<a_Dao> get_a_All() throws Exception{
            return _dal.get_a_All();
            }
		    /**
         * 根据A返回的查询DataRow创建一个AEntity对象
         * @param A row
         * @returnAList对象
        * @throws Exception 
         */
		public List<a_Dao> get_a_All(String strWhere) throws Exception{
            return _dal.get_a_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<a_Dao> get_a_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_a_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据A返回的查询DataRow创建一个AEntity对象
         * @param A row
         * @returnADictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, a_Dao> get_a_Dictionary(String strWhere) throws Exception{
            return _dal.get_a_Dictionary(strWhere);
            }
		 /**
         * 更新A字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_a_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_a_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新AInt型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_a_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_a_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新AIString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createa_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_a_UpdateString( FieldName, Value, sid);
            }
        
        
    }
