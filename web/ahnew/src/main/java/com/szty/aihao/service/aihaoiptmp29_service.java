/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataAihaoiptmp29.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.aihaoiptmp29_Dao;
import com.szty.aihao.core.aihaoiptmp29_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@AIHAOIPTMP29逻辑层接口
    *@作者：宋春林 
    */

	public class aihaoiptmp29_service
	{
        public  aihaoiptmp29_core _dal=classFactory.getaihaoiptmp29();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP29实体
         * @return 新插入记录的编号
         */
		public int insert_aihaoiptmp29 (aihaoiptmp29_Dao _AIHAOIPTMP29Model )  throws Exception{
         return _dal.insert_aihaoiptmp29( _AIHAOIPTMP29Model);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP29prrameter
         * @return 新插入记录的编号
         */
         public int insert_aihaoiptmp29(Object[] _para)  throws Exception{
        return _dal.insert_aihaoiptmp29( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP29实体
         * @return 影响的行数
         */
		public int update_aihaoiptmp29(aihaoiptmp29_Dao _AIHAOIPTMP29Model)  throws Exception{
        return _dal.update_aihaoiptmp29( _AIHAOIPTMP29Model);
		}
	
		

         /**
         *  删除数据表AIHAOIPTMP29中的一条记录
         * @param AIHAOIPTMP29实体
         * @return 新插入记录的编号
         */
		public int delete_aihaoiptmp29(int Id)  throws Exception{
        return _dal.delete_aihaoiptmp29( Id);
		}

		

       
          /**
         * 得到 aihaoiptmp29 数据实体
         * @param Id">Id
         * @return<aihaoiptmp29 数据实体
        * @throws Exception 
         */
		public aihaoiptmp29_Dao get_aihaoiptmp29Dao(int Id) throws Exception{
        return _dal.get_aihaoiptmp29Dao( Id);
		}
		
		    /**
         * 根据AIHAOIPTMP29返回的查询DataRow创建一个AIHAOIPTMP29Entity对象
         * @param AIHAOIPTMP29 row
         * @returnAIHAOIPTMP29List对象
        * @throws Exception 
         */
		public List<aihaoiptmp29_Dao> get_aihaoiptmp29_All() throws Exception{
            return _dal.get_aihaoiptmp29_All();
            }
		    /**
         * 根据AIHAOIPTMP29返回的查询DataRow创建一个AIHAOIPTMP29Entity对象
         * @param AIHAOIPTMP29 row
         * @returnAIHAOIPTMP29List对象
        * @throws Exception 
         */
		public List<aihaoiptmp29_Dao> get_aihaoiptmp29_All(String strWhere) throws Exception{
            return _dal.get_aihaoiptmp29_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<aihaoiptmp29_Dao> get_aihaoiptmp29_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_aihaoiptmp29_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据AIHAOIPTMP29返回的查询DataRow创建一个AIHAOIPTMP29Entity对象
         * @param AIHAOIPTMP29 row
         * @returnAIHAOIPTMP29Dictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, aihaoiptmp29_Dao> get_aihaoiptmp29_Dictionary(String strWhere) throws Exception{
            return _dal.get_aihaoiptmp29_Dictionary(strWhere);
            }
		 /**
         * 更新AIHAOIPTMP29字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_aihaoiptmp29_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_aihaoiptmp29_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新AIHAOIPTMP29Int型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_aihaoiptmp29_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_aihaoiptmp29_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新AIHAOIPTMP29IString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createaihaoiptmp29_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_aihaoiptmp29_UpdateString( FieldName, Value, sid);
            }
        
        
    }
