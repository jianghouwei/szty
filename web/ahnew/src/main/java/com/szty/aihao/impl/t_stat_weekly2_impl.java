...
*@作者：宋春林 
*/
public class t_stat_weekly2_impl implements t_stat_weekly2_core
{
    	private static final Logger logger = LoggerFactory
			.getLogger(t_stat_weekly2_impl.class);
...
public int insert_t_stat_weekly2(t_stat_weekly2_Dao _T_STAT_WEEKLY2Model)  throws Exception{
         String sqlStr = "{call T_STAT_WEEKLY2_PRO.InsertT_STAT_WEEKLY2(?,?,?)}";
		 OracleParameter[] op = new OracleParameter[3];
         op[0] = new OracleParameter(1, "OUT", java.sql.Types.NUMERIC,_T_STAT_WEEKLY2Model.getItem());
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_T_STAT_WEEKLY2Model.getRmk1());
        op[2]= new OracleParameter(3, "IN", java.sql.Types.NVARCHAR,_T_STAT_WEEKLY2Model.getRmk2());
...
public int update_t_stat_weekly2(t_stat_weekly2_Dao _T_STAT_WEEKLY2Model)  throws Exception{
        String sqlStr = "{call T_STAT_WEEKLY2_PRO.UpdateT_STAT_WEEKLY2(?,?,?)}";
		 OracleParameter[] op = new OracleParameter[3];
        op[0]= new OracleParameter(1, "IN", java.sql.Types.NVARCHAR,_T_STAT_WEEKLY2Model.getItem());
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_T_STAT_WEEKLY2Model.getRmk1());
        op[2]= new OracleParameter(3, "IN", java.sql.Types.NVARCHAR,_T_STAT_WEEKLY2Model.getRmk2());
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
...
public int delete_t_stat_weekly2(int Item)  throws Exception{
        String sqlStr = "{call T_STAT_WEEKLY2_PRO.DeleteT_STAT_WEEKLY2(?)}";
...
public t_stat_weekly2_Dao get_t_stat_weekly2Dao(int Item) throws Exception{
         String sqlStr = "{ call T_STAT_WEEKLY2_PRO.GetT_STAT_WEEKLY2Entity(?,?)}";
		OracleParameter[] op = new OracleParameter[2];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		op[1] = new OracleParameter(2, "IN", java.sql.Types.NUMERIC, Item);
		OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
			t_stat_weekly2_Dao _obj = new t_stat_weekly2_Dao();
...
public List<t_stat_weekly2_Dao> get_t_stat_weekly2_All() throws Exception{
        List<t_stat_weekly2_Dao> _list = new ArrayList<t_stat_weekly2_Dao>();
		String sqlStr = "{ call T_STAT_WEEKLY2_PRO.GetT_STAT_WEEKLY2(?)}";
		OracleParameter[] op = new OracleParameter[1];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
...
public List< t_stat_weekly2_Dao> get_t_stat_weekly2_All(String strWhere) throws Exception{
         List<t_stat_weekly2_Dao> _list = new ArrayList<t_stat_weekly2_Dao>();
		 String sqlStr = "{ call T_STAT_WEEKLY2_PRO.GetT_STAT_WEEKLY2Where(?,?)}";
		 OracleParameter[] op = new OracleParameter[2];
		 op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
...
public List<t_stat_weekly2_Dao> get_t_stat_weekly2_Page(int pageSize, int pageIndex,
		...
		List<t_stat_weekly2_Dao> _list = new ArrayList<t_stat_weekly2_Dao>();
		String sqlStr = "{ call T_STAT_WEEKLY2_PRO.GetT_STAT_WEEKLY2Page(?,?,?,?,?,?)}";
...
public Dictionary<Integer, t_stat_weekly2_Dao> get_t_stat_weekly2_Dictionary(String strWhere) throws Exception{
            if (classFactory.cacheGet("dict_stat_weekly2strWhere") == null) {
			Dictionary<Integer, t_stat_weekly2_Dao> _dic = new Hashtable<Integer, t_stat_weekly2_Dao>();
			List<t_stat_weekly2_Dao> _list = new ArrayList<t_stat_weekly2_Dao>();
			for (int i = 0; i < _list.size(); i++) {
				_dic.put(Integer.parseInt(_list.get(i).getItem()), _list.get(i));
			}
			classFactory.cachePut("dict_stat_weekly2", _dic);
			System.out.println("from db");
			return _dic;
		} else {
			@SuppressWarnings("unchecked")
			Dictionary<Integer, t_stat_weekly2_Dao> _dic = (Dictionary<Integer, t_stat_weekly2_Dao>) classFactory
					.cacheGet("dict_stat_weekly2strWhere");
...
public int create_t_stat_weekly2_UpdateIncreate(String FieldName,int sid)  throws Exception{
	    String sqlStr = "{ call T_STAT_WEEKLY2_PRO.UpdateFieldT_STAT_WEEKLY2(?,?)}";
		OracleParameter[] op = new OracleParameter[2];
...
{
		String sqlStr = "{ call T_STAT_WEEKLY2_PRO.UpdateFieldT_STAT_WEEKLY2V(?,?,?)}";
		OracleParameter[] op = new OracleParameter[3];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
...
{
        String sqlStr = "{ call T_STAT_WEEKLY2_PRO.UpdateFieldT_STAT_WEEKLY2U(?,?,?)}";
		OracleParameter[] op = new OracleParameter[3];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
