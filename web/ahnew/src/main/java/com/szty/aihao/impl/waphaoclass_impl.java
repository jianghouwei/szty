...
*@作者：宋春林 
*/
public class waphaoclass_impl implements waphaoclass_core
{
    	private static final Logger logger = LoggerFactory
			.getLogger(waphaoclass_impl.class);
...
public int insert_waphaoclass(waphaoclass_Dao _WAPHAOCLASSModel)  throws Exception{
         String sqlStr = "{call WAPHAOCLASS_PRO.InsertWAPHAOCLASS(?,?,?)}";
		 OracleParameter[] op = new OracleParameter[3];
         op[0] = new OracleParameter(1, "OUT", java.sql.Types.NUMERIC,_WAPHAOCLASSModel.getCat());
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_WAPHAOCLASSModel.getName());
        op[2]= new OracleParameter(3, "IN", java.sql.Types.NVARCHAR,_WAPHAOCLASSModel.getUrl());
...
public int update_waphaoclass(waphaoclass_Dao _WAPHAOCLASSModel)  throws Exception{
        String sqlStr = "{call WAPHAOCLASS_PRO.UpdateWAPHAOCLASS(?,?,?)}";
		 OracleParameter[] op = new OracleParameter[3];
        op[0]= new OracleParameter(1, "IN", java.sql.Types.NVARCHAR,_WAPHAOCLASSModel.getCat());
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_WAPHAOCLASSModel.getName());
        op[2]= new OracleParameter(3, "IN", java.sql.Types.NVARCHAR,_WAPHAOCLASSModel.getUrl());
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
...
public int delete_waphaoclass(int Cat)  throws Exception{
        String sqlStr = "{call WAPHAOCLASS_PRO.DeleteWAPHAOCLASS(?)}";
...
public waphaoclass_Dao get_waphaoclassDao(int Cat) throws Exception{
         String sqlStr = "{ call WAPHAOCLASS_PRO.GetWAPHAOCLASSEntity(?,?)}";
		OracleParameter[] op = new OracleParameter[2];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		op[1] = new OracleParameter(2, "IN", java.sql.Types.NUMERIC, Cat);
		OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
			waphaoclass_Dao _obj = new waphaoclass_Dao();
...
public List<waphaoclass_Dao> get_waphaoclass_All() throws Exception{
        List<waphaoclass_Dao> _list = new ArrayList<waphaoclass_Dao>();
		String sqlStr = "{ call WAPHAOCLASS_PRO.GetWAPHAOCLASS(?)}";
		OracleParameter[] op = new OracleParameter[1];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
...
public List< waphaoclass_Dao> get_waphaoclass_All(String strWhere) throws Exception{
         List<waphaoclass_Dao> _list = new ArrayList<waphaoclass_Dao>();
		 String sqlStr = "{ call WAPHAOCLASS_PRO.GetWAPHAOCLASSWhere(?,?)}";
		 OracleParameter[] op = new OracleParameter[2];
		 op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
...
public List<waphaoclass_Dao> get_waphaoclass_Page(int pageSize, int pageIndex,
		...
		List<waphaoclass_Dao> _list = new ArrayList<waphaoclass_Dao>();
		String sqlStr = "{ call WAPHAOCLASS_PRO.GetWAPHAOCLASSPage(?,?,?,?,?,?)}";
...
public Dictionary<Integer, waphaoclass_Dao> get_waphaoclass_Dictionary(String strWhere) throws Exception{
            if (classFactory.cacheGet("dicwaphaoclassstrWhere") == null) {
			Dictionary<Integer, waphaoclass_Dao> _dic = new Hashtable<Integer, waphaoclass_Dao>();
			List<waphaoclass_Dao> _list = new ArrayList<waphaoclass_Dao>();
			for (int i = 0; i < _list.size(); i++) {
				_dic.put(Integer.parseInt(_list.get(i).getCat()), _list.get(i));
			}
			classFactory.cachePut("dicwaphaoclass", _dic);
			System.out.println("from db");
			return _dic;
		} else {
			@SuppressWarnings("unchecked")
			Dictionary<Integer, waphaoclass_Dao> _dic = (Dictionary<Integer, waphaoclass_Dao>) classFactory
					.cacheGet("dicwaphaoclassstrWhere");
...
public int create_waphaoclass_UpdateIncreate(String FieldName,int sid)  throws Exception{
	    String sqlStr = "{ call WAPHAOCLASS_PRO.UpdateFieldWAPHAOCLASS(?,?)}";
		OracleParameter[] op = new OracleParameter[2];
...
{
		String sqlStr = "{ call WAPHAOCLASS_PRO.UpdateFieldWAPHAOCLASSV(?,?,?)}";
		OracleParameter[] op = new OracleParameter[3];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
...
{
        String sqlStr = "{ call WAPHAOCLASS_PRO.UpdateFieldWAPHAOCLASSU(?,?,?)}";
		OracleParameter[] op = new OracleParameter[3];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
