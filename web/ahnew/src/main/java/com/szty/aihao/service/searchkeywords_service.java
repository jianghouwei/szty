/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataSearchkeywords.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.searchkeywords_Dao;
import com.szty.aihao.core.searchkeywords_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@SEARCHKEYWORDS逻辑层接口
    *@作者：宋春林 
    */

	public class searchkeywords_service
	{
        public  searchkeywords_core _dal=classFactory.getsearchkeywords();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param SEARCHKEYWORDS实体
         * @return 新插入记录的编号
         */
		public int insert_searchkeywords (searchkeywords_Dao _SEARCHKEYWORDSModel )  throws Exception{
         return _dal.insert_searchkeywords( _SEARCHKEYWORDSModel);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param SEARCHKEYWORDSprrameter
         * @return 新插入记录的编号
         */
         public int insert_searchkeywords(Object[] _para)  throws Exception{
        return _dal.insert_searchkeywords( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param SEARCHKEYWORDS实体
         * @return 影响的行数
         */
		public int update_searchkeywords(searchkeywords_Dao _SEARCHKEYWORDSModel)  throws Exception{
        return _dal.update_searchkeywords( _SEARCHKEYWORDSModel);
		}
	
		

         /**
         *  删除数据表SEARCHKEYWORDS中的一条记录
         * @param SEARCHKEYWORDS实体
         * @return 新插入记录的编号
         */
		public int delete_searchkeywords(int Searchkeyid)  throws Exception{
        return _dal.delete_searchkeywords( Searchkeyid);
		}

		

       
          /**
         * 得到 searchkeywords 数据实体
         * @param Searchkeyid">Searchkeyid
         * @return<searchkeywords 数据实体
        * @throws Exception 
         */
		public searchkeywords_Dao get_searchkeywordsDao(int Searchkeyid) throws Exception{
        return _dal.get_searchkeywordsDao( Searchkeyid);
		}
		
		    /**
         * 根据SEARCHKEYWORDS返回的查询DataRow创建一个SEARCHKEYWORDSEntity对象
         * @param SEARCHKEYWORDS row
         * @returnSEARCHKEYWORDSList对象
        * @throws Exception 
         */
		public List<searchkeywords_Dao> get_searchkeywords_All() throws Exception{
            return _dal.get_searchkeywords_All();
            }
		    /**
         * 根据SEARCHKEYWORDS返回的查询DataRow创建一个SEARCHKEYWORDSEntity对象
         * @param SEARCHKEYWORDS row
         * @returnSEARCHKEYWORDSList对象
        * @throws Exception 
         */
		public List<searchkeywords_Dao> get_searchkeywords_All(String strWhere) throws Exception{
            return _dal.get_searchkeywords_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<searchkeywords_Dao> get_searchkeywords_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_searchkeywords_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据SEARCHKEYWORDS返回的查询DataRow创建一个SEARCHKEYWORDSEntity对象
         * @param SEARCHKEYWORDS row
         * @returnSEARCHKEYWORDSDictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, searchkeywords_Dao> get_searchkeywords_Dictionary(String strWhere) throws Exception{
            return _dal.get_searchkeywords_Dictionary(strWhere);
            }
		 /**
         * 更新SEARCHKEYWORDS字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_searchkeywords_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_searchkeywords_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新SEARCHKEYWORDSInt型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_searchkeywords_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_searchkeywords_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新SEARCHKEYWORDSIString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createsearchkeywords_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_searchkeywords_UpdateString( FieldName, Value, sid);
            }
        
        
    }
