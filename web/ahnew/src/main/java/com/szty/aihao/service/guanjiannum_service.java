/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataGuanjiannum.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.guanjiannum_Dao;
import com.szty.aihao.core.guanjiannum_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@GUANJIANNUM逻辑层接口
    *@作者：宋春林 
    */

	public class guanjiannum_service
	{
        public  guanjiannum_core _dal=classFactory.getguanjiannum();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param GUANJIANNUM实体
         * @return 新插入记录的编号
         */
		public int insert_guanjiannum (guanjiannum_Dao _GUANJIANNUMModel )  throws Exception{
         return _dal.insert_guanjiannum( _GUANJIANNUMModel);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param GUANJIANNUMprrameter
         * @return 新插入记录的编号
         */
         public int insert_guanjiannum(Object[] _para)  throws Exception{
        return _dal.insert_guanjiannum( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param GUANJIANNUM实体
         * @return 影响的行数
         */
		public int update_guanjiannum(guanjiannum_Dao _GUANJIANNUMModel)  throws Exception{
        return _dal.update_guanjiannum( _GUANJIANNUMModel);
		}
	
		

         /**
         *  删除数据表GUANJIANNUM中的一条记录
         * @param GUANJIANNUM实体
         * @return 新插入记录的编号
         */
		public int delete_guanjiannum(int Guanjiannum)  throws Exception{
        return _dal.delete_guanjiannum( Guanjiannum);
		}

		

       
          /**
         * 得到 guanjiannum 数据实体
         * @param Guanjiannum">Guanjiannum
         * @return<guanjiannum 数据实体
        * @throws Exception 
         */
		public guanjiannum_Dao get_guanjiannumDao(int Guanjiannum) throws Exception{
        return _dal.get_guanjiannumDao( Guanjiannum);
		}
		
		    /**
         * 根据GUANJIANNUM返回的查询DataRow创建一个GUANJIANNUMEntity对象
         * @param GUANJIANNUM row
         * @returnGUANJIANNUMList对象
        * @throws Exception 
         */
		public List<guanjiannum_Dao> get_guanjiannum_All() throws Exception{
            return _dal.get_guanjiannum_All();
            }
		    /**
         * 根据GUANJIANNUM返回的查询DataRow创建一个GUANJIANNUMEntity对象
         * @param GUANJIANNUM row
         * @returnGUANJIANNUMList对象
        * @throws Exception 
         */
		public List<guanjiannum_Dao> get_guanjiannum_All(String strWhere) throws Exception{
            return _dal.get_guanjiannum_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<guanjiannum_Dao> get_guanjiannum_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_guanjiannum_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据GUANJIANNUM返回的查询DataRow创建一个GUANJIANNUMEntity对象
         * @param GUANJIANNUM row
         * @returnGUANJIANNUMDictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, guanjiannum_Dao> get_guanjiannum_Dictionary(String strWhere) throws Exception{
            return _dal.get_guanjiannum_Dictionary(strWhere);
            }
		 /**
         * 更新GUANJIANNUM字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_guanjiannum_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_guanjiannum_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新GUANJIANNUMInt型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_guanjiannum_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_guanjiannum_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新GUANJIANNUMIString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createguanjiannum_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_guanjiannum_UpdateString( FieldName, Value, sid);
            }
        
        
    }
