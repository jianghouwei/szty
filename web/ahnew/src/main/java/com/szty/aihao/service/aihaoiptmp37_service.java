/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataAihaoiptmp37.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.aihaoiptmp37_Dao;
import com.szty.aihao.core.aihaoiptmp37_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@AIHAOIPTMP37逻辑层接口
    *@作者：宋春林 
    */

	public class aihaoiptmp37_service
	{
        public  aihaoiptmp37_core _dal=classFactory.getaihaoiptmp37();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP37实体
         * @return 新插入记录的编号
         */
		public int insert_aihaoiptmp37 (aihaoiptmp37_Dao _AIHAOIPTMP37Model )  throws Exception{
         return _dal.insert_aihaoiptmp37( _AIHAOIPTMP37Model);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP37prrameter
         * @return 新插入记录的编号
         */
         public int insert_aihaoiptmp37(Object[] _para)  throws Exception{
        return _dal.insert_aihaoiptmp37( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP37实体
         * @return 影响的行数
         */
		public int update_aihaoiptmp37(aihaoiptmp37_Dao _AIHAOIPTMP37Model)  throws Exception{
        return _dal.update_aihaoiptmp37( _AIHAOIPTMP37Model);
		}
	
		

         /**
         *  删除数据表AIHAOIPTMP37中的一条记录
         * @param AIHAOIPTMP37实体
         * @return 新插入记录的编号
         */
		public int delete_aihaoiptmp37(int Id)  throws Exception{
        return _dal.delete_aihaoiptmp37( Id);
		}

		

       
          /**
         * 得到 aihaoiptmp37 数据实体
         * @param Id">Id
         * @return<aihaoiptmp37 数据实体
        * @throws Exception 
         */
		public aihaoiptmp37_Dao get_aihaoiptmp37Dao(int Id) throws Exception{
        return _dal.get_aihaoiptmp37Dao( Id);
		}
		
		    /**
         * 根据AIHAOIPTMP37返回的查询DataRow创建一个AIHAOIPTMP37Entity对象
         * @param AIHAOIPTMP37 row
         * @returnAIHAOIPTMP37List对象
        * @throws Exception 
         */
		public List<aihaoiptmp37_Dao> get_aihaoiptmp37_All() throws Exception{
            return _dal.get_aihaoiptmp37_All();
            }
		    /**
         * 根据AIHAOIPTMP37返回的查询DataRow创建一个AIHAOIPTMP37Entity对象
         * @param AIHAOIPTMP37 row
         * @returnAIHAOIPTMP37List对象
        * @throws Exception 
         */
		public List<aihaoiptmp37_Dao> get_aihaoiptmp37_All(String strWhere) throws Exception{
            return _dal.get_aihaoiptmp37_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<aihaoiptmp37_Dao> get_aihaoiptmp37_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_aihaoiptmp37_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据AIHAOIPTMP37返回的查询DataRow创建一个AIHAOIPTMP37Entity对象
         * @param AIHAOIPTMP37 row
         * @returnAIHAOIPTMP37Dictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, aihaoiptmp37_Dao> get_aihaoiptmp37_Dictionary(String strWhere) throws Exception{
            return _dal.get_aihaoiptmp37_Dictionary(strWhere);
            }
		 /**
         * 更新AIHAOIPTMP37字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_aihaoiptmp37_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_aihaoiptmp37_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新AIHAOIPTMP37Int型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_aihaoiptmp37_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_aihaoiptmp37_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新AIHAOIPTMP37IString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createaihaoiptmp37_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_aihaoiptmp37_UpdateString( FieldName, Value, sid);
            }
        
        
    }
