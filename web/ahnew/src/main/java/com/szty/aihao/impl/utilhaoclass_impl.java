/*
*@===================================================================
*@项目说明
*@作者：宋春林 
*@版本信息:@Copy Right 2011-2015
*@文件： dataUtilhaoclass.java
*@项目名称：JAVA项目管理 
*@创建时间：2015/10/15
*@===================================================================
*/

package com.szty.aihao.impl;

import com.szty.aihao.dao.utilhaoclass_Dao;
import com.szty.aihao.core.utilhaoclass_core;
import com.szty.aihao.factory.*;
import java.util.Dictionary;
import java.util.Hashtable;
import java.util.List;
import java.util.ArrayList;
import java.text.DateFormat;
import java.text.SimpleDateFormat;
import oracle.jdbc.OracleTypes;
import oracle.jdbc.rowset.OracleCachedRowSet;
import java.util.Date;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
/**
*@文件说明
*@UTILHAOCLASS数据接口
*@作者：宋春林 
*/
public class utilhaoclass_impl implements utilhaoclass_core
{
    	private static final Logger logger = LoggerFactory
			.getLogger(utilhaoclass_impl.class);
	private static final long serialVersionUID = 1L;
	private DateFormat dateformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

/**
* 向数据库中插入一条新记录。
* @param UTILHAOCLASS实体
* @return 新插入记录的编号
*/
public int insert_utilhaoclass(utilhaoclass_Dao _UTILHAOCLASSModel)  throws Exception{
         String sqlStr = "{call UTILHAOCLASS_PRO.InsertUTILHAOCLASS(?,?)}";
		 OracleParameter[] op = new OracleParameter[2];
         op[0] = new OracleParameter(1, "OUT", java.sql.Types.NUMERIC,_UTILHAOCLASSModel.getName());
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_UTILHAOCLASSModel.getUrl());
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;

}

/**
* 向数据库中插入一条新记录。
* @param UTILHAOCLASS
* @Object 
* @return 新插入记录的编号
*/
public int insert_utilhaoclass(Object[] _para)  throws Exception{
      String sqlStr = "{call UTILHAOCLASS_PRO.InsertUTILHAOCLASS(?,?)}";
	  OracleParameter[] op = new OracleParameter[2];
       op[0] = new OracleParameter(1, "OUT", java.sql.Types.NUMERIC,_para[0]);
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_para[1]);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}

/**
* 向数据库中更新一条新记录。
* @param UTILHAOCLASS实体
* @return 影响的行数
*/
public int update_utilhaoclass(utilhaoclass_Dao _UTILHAOCLASSModel)  throws Exception{
        String sqlStr = "{call UTILHAOCLASS_PRO.UpdateUTILHAOCLASS(?,?)}";
		 OracleParameter[] op = new OracleParameter[2];
        op[0]= new OracleParameter(1, "IN", java.sql.Types.NVARCHAR,_UTILHAOCLASSModel.getName());
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_UTILHAOCLASSModel.getUrl());
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}

/**
*  删除数据表UTILHAOCLASS中的一条记录
* @param UTILHAOCLASS实体
* @return 新插入记录的编号
*/
public int delete_utilhaoclass(int Name)  throws Exception{
        String sqlStr = "{call UTILHAOCLASS_PRO.DeleteUTILHAOCLASS(?)}";
		OracleParameter[] op = new OracleParameter[1];
		op[0] = new OracleParameter(1, "IN", java.sql.Types.NUMERIC, Name);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}




/**
* 得到 utilhaoclass 数据实体
* @param Name">Name
* @return<utilhaoclass 数据实体
* @throws Exception 
*/
public utilhaoclass_Dao get_utilhaoclassDao(int Name) throws Exception{
         String sqlStr = "{ call UTILHAOCLASS_PRO.GetUTILHAOCLASSEntity(?,?)}";
		OracleParameter[] op = new OracleParameter[2];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		op[1] = new OracleParameter(2, "IN", java.sql.Types.NUMERIC, Name);
		OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
			utilhaoclass_Dao _obj = new utilhaoclass_Dao();
            _obj.setName(arr.getString("NAME"));
            _obj.setUrl(arr.getString("URL"));
			return _obj;
		}
		return null;

}

/**
* 根据UTILHAOCLASS返回的查询DataRow创建一个UTILHAOCLASSEntity对象
* @param UTILHAOCLASS row
* @returnUTILHAOCLASSList对象
* @throws Exception 
*/
public List<utilhaoclass_Dao> get_utilhaoclass_All() throws Exception{
        List<utilhaoclass_Dao> _list = new ArrayList<utilhaoclass_Dao>();
		String sqlStr = "{ call UTILHAOCLASS_PRO.GetUTILHAOCLASS(?)}";
		OracleParameter[] op = new OracleParameter[1];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
			utilhaoclass_Dao _obj = new utilhaoclass_Dao();
            _obj.setName(arr.getString("NAME"));
            _obj.setUrl(arr.getString("URL"));
            _list.add(_obj);
		}
		return _list;
}
/**
* 根据UTILHAOCLASS返回的查询DataRow创建一个UTILHAOCLASSEntity对象
* @param UTILHAOCLASS row
* @returnUTILHAOCLASSList对象
* @throws Exception 
*/
public List< utilhaoclass_Dao> get_utilhaoclass_All(String strWhere) throws Exception{
         List<utilhaoclass_Dao> _list = new ArrayList<utilhaoclass_Dao>();
		 String sqlStr = "{ call UTILHAOCLASS_PRO.GetUTILHAOCLASSWhere(?,?)}";
		 OracleParameter[] op = new OracleParameter[2];
		 op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		 op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, strWhere);
		 OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
				utilhaoclass_Dao _obj = new utilhaoclass_Dao();
            _obj.setName(arr.getString("NAME"));
            _obj.setUrl(arr.getString("URL"));
            _list.add(_obj);
		}
		return _list;
}

/**
* 根据UTILHAOCLASS返回 分页数据
* @param UTILHAOCLASS row
* @returnUTILHAOCLASSList对象
* @throws Exception 
*/
public List<utilhaoclass_Dao> get_utilhaoclass_Page(int pageSize, int pageIndex,
			String strWhere) throws Exception {
		List<utilhaoclass_Dao> _list = new ArrayList<utilhaoclass_Dao>();
		String sqlStr = "{ call UTILHAOCLASS_PRO.GetUTILHAOCLASSPage(?,?,?,?,?,?)}";
		OracleParameter[] op = new OracleParameter[6];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		op[1] = new OracleParameter(2, "OUT", OracleTypes.NUMBER, "");
		op[2] = new OracleParameter(3, "OUT", OracleTypes.NUMBER, "");
		op[3] = new OracleParameter(4, "IN", OracleTypes.NUMBER, pageIndex);
		op[4] = new OracleParameter(5, "IN", OracleTypes.NUMBER, pageSize);
		op[5] = new OracleParameter(6, "IN", OracleTypes.VARCHAR, strWhere);
		OraclePageCache opc = OracleHelper.executeQueryPage(sqlStr, op);
		OracleCachedRowSet arr = opc.getOcrs();
		while (arr.next()) {
				utilhaoclass_Dao _obj = new utilhaoclass_Dao();
            _obj.setName(arr.getString("NAME"));
            _obj.setUrl(arr.getString("URL"));
			_list.add(_obj);
		}
		return _list;
	}
/**
* 根据UTILHAOCLASS返回的查询DataRow创建一个UTILHAOCLASSEntity对象
* @param UTILHAOCLASS row
* @returnUTILHAOCLASSDictionary对象
* @throws Exception 
*/
public Dictionary<Integer, utilhaoclass_Dao> get_utilhaoclass_Dictionary(String strWhere) throws Exception{
            if (classFactory.cacheGet("dicutilhaoclassstrWhere") == null) {
			Dictionary<Integer, utilhaoclass_Dao> _dic = new Hashtable<Integer, utilhaoclass_Dao>();
			List<utilhaoclass_Dao> _list = new ArrayList<utilhaoclass_Dao>();
			for (int i = 0; i < _list.size(); i++) {
				_dic.put(Integer.parseInt(_list.get(i).getName()), _list.get(i));
			}
			classFactory.cachePut("dicutilhaoclass", _dic);
			System.out.println("from db");
			return _dic;
		} else {
			@SuppressWarnings("unchecked")
			Dictionary<Integer, utilhaoclass_Dao> _dic = (Dictionary<Integer, utilhaoclass_Dao>) classFactory
					.cacheGet("dicutilhaoclassstrWhere");
			System.out.println("from cache");
			return _dic;
		}
}
/**
* 更新UTILHAOCLASS字段加一
* @param FieldName
* @param sid
*/
public int create_utilhaoclass_UpdateIncreate(String FieldName,int sid)  throws Exception{
	    String sqlStr = "{ call UTILHAOCLASS_PRO.UpdateFieldUTILHAOCLASS(?,?)}";
		OracleParameter[] op = new OracleParameter[2];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}
/**
* 更新UTILHAOCLASSInt型字段
* @param FieldName
* @param Num
* @param sid
*/
public int create_utilhaoclass_UpdateInteger(String FieldName,int Num,int sid)  throws Exception
{
		String sqlStr = "{ call UTILHAOCLASS_PRO.UpdateFieldUTILHAOCLASSV(?,?,?)}";
		OracleParameter[] op = new OracleParameter[3];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
		op[2] = new OracleParameter(3, "IN", OracleTypes.NUMBER, Num);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;


}
/**
* 更新UTILHAOCLASSIString型字段
* @param FieldName
* @param Value
* @param sid
*/
public int create_utilhaoclass_UpdateString(String FieldName,String Value,int sid)  throws Exception
{
        String sqlStr = "{ call UTILHAOCLASS_PRO.UpdateFieldUTILHAOCLASSU(?,?,?)}";
		OracleParameter[] op = new OracleParameter[3];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
		op[2] = new OracleParameter(3, "IN", OracleTypes.VARCHAR, Value);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}



}
