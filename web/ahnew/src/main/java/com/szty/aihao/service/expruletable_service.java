/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataExpruletable.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.expruletable_Dao;
import com.szty.aihao.core.expruletable_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@EXPRULETABLE逻辑层接口
    *@作者：宋春林 
    */

	public class expruletable_service
	{
        public  expruletable_core _dal=classFactory.getexpruletable();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param EXPRULETABLE实体
         * @return 新插入记录的编号
         */
		public int insert_expruletable (expruletable_Dao _EXPRULETABLEModel )  throws Exception{
         return _dal.insert_expruletable( _EXPRULETABLEModel);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param EXPRULETABLEprrameter
         * @return 新插入记录的编号
         */
         public int insert_expruletable(Object[] _para)  throws Exception{
        return _dal.insert_expruletable( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param EXPRULETABLE实体
         * @return 影响的行数
         */
		public int update_expruletable(expruletable_Dao _EXPRULETABLEModel)  throws Exception{
        return _dal.update_expruletable( _EXPRULETABLEModel);
		}
	
		

         /**
         *  删除数据表EXPRULETABLE中的一条记录
         * @param EXPRULETABLE实体
         * @return 新插入记录的编号
         */
		public int delete_expruletable(int Ruleid)  throws Exception{
        return _dal.delete_expruletable( Ruleid);
		}

		

       
          /**
         * 得到 expruletable 数据实体
         * @param Ruleid">Ruleid
         * @return<expruletable 数据实体
        * @throws Exception 
         */
		public expruletable_Dao get_expruletableDao(int Ruleid) throws Exception{
        return _dal.get_expruletableDao( Ruleid);
		}
		
		    /**
         * 根据EXPRULETABLE返回的查询DataRow创建一个EXPRULETABLEEntity对象
         * @param EXPRULETABLE row
         * @returnEXPRULETABLEList对象
        * @throws Exception 
         */
		public List<expruletable_Dao> get_expruletable_All() throws Exception{
            return _dal.get_expruletable_All();
            }
		    /**
         * 根据EXPRULETABLE返回的查询DataRow创建一个EXPRULETABLEEntity对象
         * @param EXPRULETABLE row
         * @returnEXPRULETABLEList对象
        * @throws Exception 
         */
		public List<expruletable_Dao> get_expruletable_All(String strWhere) throws Exception{
            return _dal.get_expruletable_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<expruletable_Dao> get_expruletable_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_expruletable_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据EXPRULETABLE返回的查询DataRow创建一个EXPRULETABLEEntity对象
         * @param EXPRULETABLE row
         * @returnEXPRULETABLEDictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, expruletable_Dao> get_expruletable_Dictionary(String strWhere) throws Exception{
            return _dal.get_expruletable_Dictionary(strWhere);
            }
		 /**
         * 更新EXPRULETABLE字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_expruletable_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_expruletable_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新EXPRULETABLEInt型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_expruletable_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_expruletable_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新EXPRULETABLEIString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createexpruletable_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_expruletable_UpdateString( FieldName, Value, sid);
            }
        
        
    }
