/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataAihaoiptmp13.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.aihaoiptmp13_Dao;
import com.szty.aihao.core.aihaoiptmp13_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@AIHAOIPTMP13逻辑层接口
    *@作者：宋春林 
    */

	public class aihaoiptmp13_service
	{
        public  aihaoiptmp13_core _dal=classFactory.getaihaoiptmp13();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP13实体
         * @return 新插入记录的编号
         */
		public int insert_aihaoiptmp13 (aihaoiptmp13_Dao _AIHAOIPTMP13Model )  throws Exception{
         return _dal.insert_aihaoiptmp13( _AIHAOIPTMP13Model);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP13prrameter
         * @return 新插入记录的编号
         */
         public int insert_aihaoiptmp13(Object[] _para)  throws Exception{
        return _dal.insert_aihaoiptmp13( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param AIHAOIPTMP13实体
         * @return 影响的行数
         */
		public int update_aihaoiptmp13(aihaoiptmp13_Dao _AIHAOIPTMP13Model)  throws Exception{
        return _dal.update_aihaoiptmp13( _AIHAOIPTMP13Model);
		}
	
		

         /**
         *  删除数据表AIHAOIPTMP13中的一条记录
         * @param AIHAOIPTMP13实体
         * @return 新插入记录的编号
         */
		public int delete_aihaoiptmp13(int Id)  throws Exception{
        return _dal.delete_aihaoiptmp13( Id);
		}

		

       
          /**
         * 得到 aihaoiptmp13 数据实体
         * @param Id">Id
         * @return<aihaoiptmp13 数据实体
        * @throws Exception 
         */
		public aihaoiptmp13_Dao get_aihaoiptmp13Dao(int Id) throws Exception{
        return _dal.get_aihaoiptmp13Dao( Id);
		}
		
		    /**
         * 根据AIHAOIPTMP13返回的查询DataRow创建一个AIHAOIPTMP13Entity对象
         * @param AIHAOIPTMP13 row
         * @returnAIHAOIPTMP13List对象
        * @throws Exception 
         */
		public List<aihaoiptmp13_Dao> get_aihaoiptmp13_All() throws Exception{
            return _dal.get_aihaoiptmp13_All();
            }
		    /**
         * 根据AIHAOIPTMP13返回的查询DataRow创建一个AIHAOIPTMP13Entity对象
         * @param AIHAOIPTMP13 row
         * @returnAIHAOIPTMP13List对象
        * @throws Exception 
         */
		public List<aihaoiptmp13_Dao> get_aihaoiptmp13_All(String strWhere) throws Exception{
            return _dal.get_aihaoiptmp13_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<aihaoiptmp13_Dao> get_aihaoiptmp13_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_aihaoiptmp13_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据AIHAOIPTMP13返回的查询DataRow创建一个AIHAOIPTMP13Entity对象
         * @param AIHAOIPTMP13 row
         * @returnAIHAOIPTMP13Dictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, aihaoiptmp13_Dao> get_aihaoiptmp13_Dictionary(String strWhere) throws Exception{
            return _dal.get_aihaoiptmp13_Dictionary(strWhere);
            }
		 /**
         * 更新AIHAOIPTMP13字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_aihaoiptmp13_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_aihaoiptmp13_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新AIHAOIPTMP13Int型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_aihaoiptmp13_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_aihaoiptmp13_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新AIHAOIPTMP13IString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createaihaoiptmp13_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_aihaoiptmp13_UpdateString( FieldName, Value, sid);
            }
        
        
    }
