/*
*@===================================================================
*@项目说明
*@作者：宋春林 
*@版本信息:@Copy Right 2011-2015
*@文件： dataAihaoverifycode.java
*@项目名称：JAVA项目管理 
*@创建时间：2015/10/15
*@===================================================================
*/

package com.szty.aihao.impl;

import com.szty.aihao.dao.aihaoverifycode_Dao;
import com.szty.aihao.core.aihaoverifycode_core;
import com.szty.aihao.factory.*;
import java.util.Dictionary;
import java.util.Hashtable;
import java.util.List;
import java.util.ArrayList;
import java.text.DateFormat;
import java.text.SimpleDateFormat;
import oracle.jdbc.OracleTypes;
import oracle.jdbc.rowset.OracleCachedRowSet;
import java.util.Date;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
/**
*@文件说明
*@AIHAOVERIFYCODE数据接口
*@作者：宋春林 
*/
public class aihaoverifycode_impl implements aihaoverifycode_core
{
    	private static final Logger logger = LoggerFactory
			.getLogger(aihaoverifycode_impl.class);
	private static final long serialVersionUID = 1L;
	private DateFormat dateformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

/**
* 向数据库中插入一条新记录。
* @param AIHAOVERIFYCODE实体
* @return 新插入记录的编号
*/
public int insert_aihaoverifycode(aihaoverifycode_Dao _AIHAOVERIFYCODEModel)  throws Exception{
         String sqlStr = "{call AIHAOVERIFYCODE_PRO.InsertAIHAOVERIFYCODE(?,?,?,?,?)}";
		 OracleParameter[] op = new OracleParameter[5];
         op[0] = new OracleParameter(1, "OUT", java.sql.Types.NUMERIC,_AIHAOVERIFYCODEModel.getId());
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_AIHAOVERIFYCODEModel.getVerifycode());
        op[2]= new OracleParameter(3, "IN", java.sql.Types.NVARCHAR,_AIHAOVERIFYCODEModel.getMobile());
        op[3]= new OracleParameter(4, "IN", java.sql.Types.DATE, dateformat.format(_AIHAOVERIFYCODEModel.getCreatetime()));
        op[4]= new OracleParameter(5, "IN", java.sql.Types.DATE, dateformat.format(_AIHAOVERIFYCODEModel.getValiditytime()));
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;

}

/**
* 向数据库中插入一条新记录。
* @param AIHAOVERIFYCODE
* @Object 
* @return 新插入记录的编号
*/
public int insert_aihaoverifycode(Object[] _para)  throws Exception{
      String sqlStr = "{call AIHAOVERIFYCODE_PRO.InsertAIHAOVERIFYCODE(?,?,?,?,?)}";
	  OracleParameter[] op = new OracleParameter[5];
       op[0] = new OracleParameter(1, "OUT", java.sql.Types.NUMERIC,_para[0]);
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_para[1]);
        op[2]= new OracleParameter(3, "IN", java.sql.Types.NVARCHAR,_para[2]);
        op[3]= new OracleParameter(4, "IN", java.sql.Types.DATE, dateformat.format(_para[3]));
        op[4]= new OracleParameter(5, "IN", java.sql.Types.DATE, dateformat.format(_para[4]));
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}

/**
* 向数据库中更新一条新记录。
* @param AIHAOVERIFYCODE实体
* @return 影响的行数
*/
public int update_aihaoverifycode(aihaoverifycode_Dao _AIHAOVERIFYCODEModel)  throws Exception{
        String sqlStr = "{call AIHAOVERIFYCODE_PRO.UpdateAIHAOVERIFYCODE(?,?,?,?,?)}";
		 OracleParameter[] op = new OracleParameter[5];
        op[0]= new OracleParameter(1, "IN", java.sql.Types.NVARCHAR,_AIHAOVERIFYCODEModel.getId());
        op[1]= new OracleParameter(2, "IN", java.sql.Types.NVARCHAR,_AIHAOVERIFYCODEModel.getVerifycode());
        op[2]= new OracleParameter(3, "IN", java.sql.Types.NVARCHAR,_AIHAOVERIFYCODEModel.getMobile());
        op[3]= new OracleParameter(4, "IN", java.sql.Types.DATE, dateformat.format(_AIHAOVERIFYCODEModel.getCreatetime()));
        op[4]= new OracleParameter(5, "IN", java.sql.Types.DATE, dateformat.format(_AIHAOVERIFYCODEModel.getValiditytime()));
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}

/**
*  删除数据表AIHAOVERIFYCODE中的一条记录
* @param AIHAOVERIFYCODE实体
* @return 新插入记录的编号
*/
public int delete_aihaoverifycode(int Id)  throws Exception{
        String sqlStr = "{call AIHAOVERIFYCODE_PRO.DeleteAIHAOVERIFYCODE(?)}";
		OracleParameter[] op = new OracleParameter[1];
		op[0] = new OracleParameter(1, "IN", java.sql.Types.NUMERIC, Id);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}




/**
* 得到 aihaoverifycode 数据实体
* @param Id">Id
* @return<aihaoverifycode 数据实体
* @throws Exception 
*/
public aihaoverifycode_Dao get_aihaoverifycodeDao(int Id) throws Exception{
         String sqlStr = "{ call AIHAOVERIFYCODE_PRO.GetAIHAOVERIFYCODEEntity(?,?)}";
		OracleParameter[] op = new OracleParameter[2];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		op[1] = new OracleParameter(2, "IN", java.sql.Types.NUMERIC, Id);
		OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
			aihaoverifycode_Dao _obj = new aihaoverifycode_Dao();
            _obj.setId(arr.getString("ID"));
            _obj.setVerifycode(arr.getString("VERIFYCODE"));
            _obj.setMobile(arr.getString("MOBILE"));
             try{_obj.setCreatetime(dateformat.parse(arr.getString("CREATETIME")));}catch(Exception ce){_obj.setCreatetime((new Date()));}
             try{_obj.setValiditytime(dateformat.parse(arr.getString("VALIDITYTIME")));}catch(Exception ce){_obj.setValiditytime((new Date()));}
			return _obj;
		}
		return null;

}

/**
* 根据AIHAOVERIFYCODE返回的查询DataRow创建一个AIHAOVERIFYCODEEntity对象
* @param AIHAOVERIFYCODE row
* @returnAIHAOVERIFYCODEList对象
* @throws Exception 
*/
public List<aihaoverifycode_Dao> get_aihaoverifycode_All() throws Exception{
        List<aihaoverifycode_Dao> _list = new ArrayList<aihaoverifycode_Dao>();
		String sqlStr = "{ call AIHAOVERIFYCODE_PRO.GetAIHAOVERIFYCODE(?)}";
		OracleParameter[] op = new OracleParameter[1];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
			aihaoverifycode_Dao _obj = new aihaoverifycode_Dao();
            _obj.setId(arr.getString("ID"));
            _obj.setVerifycode(arr.getString("VERIFYCODE"));
            _obj.setMobile(arr.getString("MOBILE"));
            try{_obj.setCreatetime(dateformat.parse(arr.getString("CREATETIME")));}catch(Exception ce){_obj.setCreatetime((new Date()));}
            try{_obj.setValiditytime(dateformat.parse(arr.getString("VALIDITYTIME")));}catch(Exception ce){_obj.setValiditytime((new Date()));}
            _list.add(_obj);
		}
		return _list;
}
/**
* 根据AIHAOVERIFYCODE返回的查询DataRow创建一个AIHAOVERIFYCODEEntity对象
* @param AIHAOVERIFYCODE row
* @returnAIHAOVERIFYCODEList对象
* @throws Exception 
*/
public List< aihaoverifycode_Dao> get_aihaoverifycode_All(String strWhere) throws Exception{
         List<aihaoverifycode_Dao> _list = new ArrayList<aihaoverifycode_Dao>();
		 String sqlStr = "{ call AIHAOVERIFYCODE_PRO.GetAIHAOVERIFYCODEWhere(?,?)}";
		 OracleParameter[] op = new OracleParameter[2];
		 op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		 op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, strWhere);
		 OracleCachedRowSet arr = OracleHelper.executeQuery(sqlStr, op);
		while (arr.next()) {
				aihaoverifycode_Dao _obj = new aihaoverifycode_Dao();
            _obj.setId(arr.getString("ID"));
            _obj.setVerifycode(arr.getString("VERIFYCODE"));
            _obj.setMobile(arr.getString("MOBILE"));
           try{_obj.setCreatetime(dateformat.parse(arr.getString("CREATETIME")));}catch(Exception ce){_obj.setCreatetime((new Date()));}
           try{_obj.setValiditytime(dateformat.parse(arr.getString("VALIDITYTIME")));}catch(Exception ce){_obj.setValiditytime((new Date()));}
            _list.add(_obj);
		}
		return _list;
}

/**
* 根据AIHAOVERIFYCODE返回 分页数据
* @param AIHAOVERIFYCODE row
* @returnAIHAOVERIFYCODEList对象
* @throws Exception 
*/
public List<aihaoverifycode_Dao> get_aihaoverifycode_Page(int pageSize, int pageIndex,
			String strWhere) throws Exception {
		List<aihaoverifycode_Dao> _list = new ArrayList<aihaoverifycode_Dao>();
		String sqlStr = "{ call AIHAOVERIFYCODE_PRO.GetAIHAOVERIFYCODEPage(?,?,?,?,?,?)}";
		OracleParameter[] op = new OracleParameter[6];
		op[0] = new OracleParameter(1, "OUT", OracleTypes.CURSOR, "");
		op[1] = new OracleParameter(2, "OUT", OracleTypes.NUMBER, "");
		op[2] = new OracleParameter(3, "OUT", OracleTypes.NUMBER, "");
		op[3] = new OracleParameter(4, "IN", OracleTypes.NUMBER, pageIndex);
		op[4] = new OracleParameter(5, "IN", OracleTypes.NUMBER, pageSize);
		op[5] = new OracleParameter(6, "IN", OracleTypes.VARCHAR, strWhere);
		OraclePageCache opc = OracleHelper.executeQueryPage(sqlStr, op);
		OracleCachedRowSet arr = opc.getOcrs();
		while (arr.next()) {
				aihaoverifycode_Dao _obj = new aihaoverifycode_Dao();
            _obj.setId(arr.getString("ID"));
            _obj.setVerifycode(arr.getString("VERIFYCODE"));
            _obj.setMobile(arr.getString("MOBILE"));
            try{_obj.setCreatetime(dateformat.parse(arr.getString("CREATETIME")));}catch(Exception ce){_obj.setCreatetime((new Date()));}
            try{_obj.setValiditytime(dateformat.parse(arr.getString("VALIDITYTIME")));}catch(Exception ce){_obj.setValiditytime((new Date()));}
			_list.add(_obj);
		}
		return _list;
	}
/**
* 根据AIHAOVERIFYCODE返回的查询DataRow创建一个AIHAOVERIFYCODEEntity对象
* @param AIHAOVERIFYCODE row
* @returnAIHAOVERIFYCODEDictionary对象
* @throws Exception 
*/
public Dictionary<Integer, aihaoverifycode_Dao> get_aihaoverifycode_Dictionary(String strWhere) throws Exception{
            if (classFactory.cacheGet("dicaihaoverifycodestrWhere") == null) {
			Dictionary<Integer, aihaoverifycode_Dao> _dic = new Hashtable<Integer, aihaoverifycode_Dao>();
			List<aihaoverifycode_Dao> _list = new ArrayList<aihaoverifycode_Dao>();
			for (int i = 0; i < _list.size(); i++) {
				_dic.put(Integer.parseInt(_list.get(i).getId()), _list.get(i));
			}
			classFactory.cachePut("dicaihaoverifycode", _dic);
			System.out.println("from db");
			return _dic;
		} else {
			@SuppressWarnings("unchecked")
			Dictionary<Integer, aihaoverifycode_Dao> _dic = (Dictionary<Integer, aihaoverifycode_Dao>) classFactory
					.cacheGet("dicaihaoverifycodestrWhere");
			System.out.println("from cache");
			return _dic;
		}
}
/**
* 更新AIHAOVERIFYCODE字段加一
* @param FieldName
* @param sid
*/
public int create_aihaoverifycode_UpdateIncreate(String FieldName,int sid)  throws Exception{
	    String sqlStr = "{ call AIHAOVERIFYCODE_PRO.UpdateFieldAIHAOVERIFYCODE(?,?)}";
		OracleParameter[] op = new OracleParameter[2];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}
/**
* 更新AIHAOVERIFYCODEInt型字段
* @param FieldName
* @param Num
* @param sid
*/
public int create_aihaoverifycode_UpdateInteger(String FieldName,int Num,int sid)  throws Exception
{
		String sqlStr = "{ call AIHAOVERIFYCODE_PRO.UpdateFieldAIHAOVERIFYCODEV(?,?,?)}";
		OracleParameter[] op = new OracleParameter[3];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
		op[2] = new OracleParameter(3, "IN", OracleTypes.NUMBER, Num);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;


}
/**
* 更新AIHAOVERIFYCODEIString型字段
* @param FieldName
* @param Value
* @param sid
*/
public int create_aihaoverifycode_UpdateString(String FieldName,String Value,int sid)  throws Exception
{
        String sqlStr = "{ call AIHAOVERIFYCODE_PRO.UpdateFieldAIHAOVERIFYCODEU(?,?,?)}";
		OracleParameter[] op = new OracleParameter[3];
		op[0] = new OracleParameter(1, "IN", OracleTypes.NUMBER, sid);
		op[1] = new OracleParameter(2, "IN", OracleTypes.VARCHAR, FieldName);
		op[2] = new OracleParameter(3, "IN", OracleTypes.VARCHAR, Value);
		boolean result = OracleHelper.executeProcedure(sqlStr.toString(), op);
		if (result)
			return 0;
		else
			return -1;
}



}
