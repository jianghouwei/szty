/*
 *@===================================================================
 *@项目说明
 *@作者：宋春林 
 *@版本信息:@Copy Right 2011-2015
 *@文件： iDataAihaobusiness15.java
 *@项目名称：JAVA项目管理
 *@创建时间：2015/10/15
 *@===================================================================
 */

package com.szty.aihao.service;

import com.szty.aihao.dao.aihaobusiness15_Dao;
import com.szty.aihao.core.aihaobusiness15_core;
import com.szty.aihao.factory.classFactory;
import java.util.Dictionary;
import java.util.List;


    /**
    *@文件说明
    *@AIHAOBUSINESS15逻辑层接口
    *@作者：宋春林 
    */

	public class aihaobusiness15_service
	{
        public  aihaobusiness15_core _dal=classFactory.getaihaobusiness15();
	
     
         /**
         * 向数据库中插入一条新记录。
         * @param AIHAOBUSINESS15实体
         * @return 新插入记录的编号
         */
		public int insert_aihaobusiness15 (aihaobusiness15_Dao _AIHAOBUSINESS15Model )  throws Exception{
         return _dal.insert_aihaobusiness15( _AIHAOBUSINESS15Model);
		}
        
        /**
         * 向数据库中插入一条新记录。
         * @param AIHAOBUSINESS15prrameter
         * @return 新插入记录的编号
         */
         public int insert_aihaobusiness15(Object[] _para)  throws Exception{
        return _dal.insert_aihaobusiness15( _para);
        }
	
		 /**
         * 向数据库中插入一条新记录。
         * @param AIHAOBUSINESS15实体
         * @return 影响的行数
         */
		public int update_aihaobusiness15(aihaobusiness15_Dao _AIHAOBUSINESS15Model)  throws Exception{
        return _dal.update_aihaobusiness15( _AIHAOBUSINESS15Model);
		}
	
		

         /**
         *  删除数据表AIHAOBUSINESS15中的一条记录
         * @param AIHAOBUSINESS15实体
         * @return 新插入记录的编号
         */
		public int delete_aihaobusiness15(int Sname)  throws Exception{
        return _dal.delete_aihaobusiness15( Sname);
		}

		

       
          /**
         * 得到 aihaobusiness15 数据实体
         * @param Sname">Sname
         * @return<aihaobusiness15 数据实体
        * @throws Exception 
         */
		public aihaobusiness15_Dao get_aihaobusiness15Dao(int Sname) throws Exception{
        return _dal.get_aihaobusiness15Dao( Sname);
		}
		
		    /**
         * 根据AIHAOBUSINESS15返回的查询DataRow创建一个AIHAOBUSINESS15Entity对象
         * @param AIHAOBUSINESS15 row
         * @returnAIHAOBUSINESS15List对象
        * @throws Exception 
         */
		public List<aihaobusiness15_Dao> get_aihaobusiness15_All() throws Exception{
            return _dal.get_aihaobusiness15_All();
            }
		    /**
         * 根据AIHAOBUSINESS15返回的查询DataRow创建一个AIHAOBUSINESS15Entity对象
         * @param AIHAOBUSINESS15 row
         * @returnAIHAOBUSINESS15List对象
        * @throws Exception 
         */
		public List<aihaobusiness15_Dao> get_aihaobusiness15_All(String strWhere) throws Exception{
            return _dal.get_aihaobusiness15_All(strWhere);
            } 
     /* 根据SCLTEST返回 分页数据
	 * 
	 * @param SCLTEST
	 *            row
	 * @returnSCLTESTList对象
	 * @throws Exception
	 */
	public List<aihaobusiness15_Dao> get_aihaobusiness15_Page(int pageSize, int pageIndex,String strWhere) throws Exception 
            {
                return _dal.get_aihaobusiness15_Page(pageSize,pageIndex,strWhere);
            }
           /**
         * 根据AIHAOBUSINESS15返回的查询DataRow创建一个AIHAOBUSINESS15Entity对象
         * @param AIHAOBUSINESS15 row
         * @returnAIHAOBUSINESS15Dictionary对象
        * @throws Exception 
         */
        public Dictionary<Integer, aihaobusiness15_Dao> get_aihaobusiness15_Dictionary(String strWhere) throws Exception{
            return _dal.get_aihaobusiness15_Dictionary(strWhere);
            }
		 /**
         * 更新AIHAOBUSINESS15字段加一
         * @param FieldName
         * @param sid
         */
	    public int create_aihaobusiness15_UpdateIncreate(String FieldName,int sid)  throws Exception{
            return _dal.create_aihaobusiness15_UpdateIncreate( FieldName, sid);
            }
         /**
         * 更新AIHAOBUSINESS15Int型字段
         * @param FieldName
         * @param Num
         * @param sid
         */
	    public int create_aihaobusiness15_UpdateInteger(String FieldName,int Num,int sid)  throws Exception{
            return _dal.create_aihaobusiness15_UpdateInteger( FieldName, Num, sid);
            }
          /**
         * 更新AIHAOBUSINESS15IString型字段
         * @param FieldName
         * @param Value
         * @param sid
         */
	    public int createaihaobusiness15_UpdateString(String FieldName,String Value,int sid)  throws Exception{
            
            return _dal.create_aihaobusiness15_UpdateString( FieldName, Value, sid);
            }
        
        
    }
