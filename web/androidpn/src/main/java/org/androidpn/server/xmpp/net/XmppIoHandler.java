...
public class XmppIoHandler implements IoHandler {
	 ...
	 * created.
	 * 
	 * IoSession对象被创建时的回调，一般用于进行会话初始化操作。 
	 * 注意：与sessionOpened(IoSession)不同，
	 * IoSession对象的创建并不意味着对应底层TCP连接的建立， 
	 * 而仅仅代表字面意思：一个IoSession对象被创建出来了。
	 */
	 ...
	 * 
	 * IoSession对象被打开时回调。在TCP中， 
	 * 该事件是在TCP连接建立时触发，
	 * 一般可用于发起连接建立的握手、认证等操作。
	 */
	...
	/**
	 * IoSession对象超时时回调。当一个IoSession对象在指定的超时时常内没有读写事件发生，
	 * 就会触发该事件，
	 * 一般可用于通知服务器断开长时间闲置的连接等处理。
	 * 具体的超时设置可由 IoService.setWriteIdleTime(int)
