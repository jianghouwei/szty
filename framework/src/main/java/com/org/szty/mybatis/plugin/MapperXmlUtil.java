...
public class MapperXmlUtil {
	...
	public static void buildSQLNew(BufferedWriter bw, List<String> columns, List<String> types) throws IOException {
		int size = columns.size();
		...
		bw.write(" \t\t </set>");
		bw.newLine();
		bw.write("\t\t WHERE " + columns.get(0) + " = #{" + columns.get(0) // processField(columns.get(0))
				+ "}");
		bw.newLine();
		bw.write("\t</update>");
		bw.newLine();
		...
		bw.write("\t<!-- 查询  List 查询 -->");
		bw.newLine();
		bw.write("\t<select id=\"queryList\" resultMap=\"" + mapname + "\" parameterType=\"" + TabClassUtil.modelName
				+ "\">");
		bw.newLine();
		bw.write("\t\t SELECT");
		bw.newLine();
